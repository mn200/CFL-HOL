...
Q_TAC SUFF_TAC `FINITE (newAprods A B ru ∪ bprods A B ru)` THEN1
SRW_TAC [][] THEN
`∀N B. { rule N B | F } = {}` by SRW_TAC [][EXTENSION] THEN
...
Q_TAC SUFF_TAC `Horrible = BIGUNION (IMAGE f ru)`
 THEN1 (DISCH_THEN SUBST1_TAC THEN SRW_TAC [][Abbr`f`] THEN
	...
	Cases_on `∃a.l = NTS A::a` THEN FULL_SIMP_TAC (srw_ss()) [] THEN
	SRW_TAC [][] THEN
	Q.MATCH_ABBREV_TAC `FINITE FOO` THEN1
	(Q_TAC SUFF_TAC `FOO = {rule B a}` THEN
	SRW_TAC [][] THEN SRW_TAC [][Abbr`FOO`, EXTENSION]) THEN1
	(Q_TAC SUFF_TAC `FOO = {rule B (a ++ [NTS B]) }` THEN
	SRW_TAC [][] THEN SRW_TAC [][Abbr`FOO`, EXTENSION]) THEN1
	(Q_TAC SUFF_TAC `FOO = {}` THEN
	 SRW_TAC [][] THEN SRW_TAC [][Abbr`FOO`, EXTENSION]) THEN
	(Q_TAC SUFF_TAC `FOO = { rule A (l ++ [NTS B]) }` THEN
	 SRW_TAC [][] THEN SRW_TAC [][Abbr`FOO`, EXTENSION])) THEN
   ...
   FULL_SIMP_TAC (srw_ss()) [recprods, newAprods, bprods] THEN
...
``∀dl x y.
     lderives g ⊢ dl ◁ x → y ∧ 
     ldNumNt (NTS A) dl ≠ 0 ∧ LENGTH dl > 1
    ⇒
     ∃dl1 dl2 dl3. 
        (dl = dl1++dl2++dl3) ∧ (ldNumNt (NTS A) dl1 = 0) ∧
        (∀e1 e2 p s.(dl2 = p ++ [e1; e2] ++ s) ⇒ LENGTH e2 ≥ LENGTH e1) ∧
        ∃pfx. EVERY isTmnlSym pfx  ∧ 
               (∀e.MEM e dl2 ⇒ ∃sfx.(e=pfx++[NTS A]++sfx)) ∧ dl2≠[] ∧
...
val elemNotNilRd = store_thm
("elemNotNilRd",
``∀dl' e.rderives g ⊢ dl' ◁ e → LAST dl' ∧ e ≠ [] ∧
 (∀e1 e2 p s. (dl' = p ++ [e1; e2] ++ s) ⇒ LENGTH e2 ≥ LENGTH e1)
⇒
(LAST dl' ≠ [])``,
...
val g'ImpgSing = store_thm
("g'ImpgSing",
``left2Right A B g g' ∧
rderives g' x (pfx ++ [NTS B] ++ sfx) ∧ ¬(NTS B ∈ x) ∧
rderives g' (pfx ++ [NTS B] ++ sfx) y' ∧ isWord sfx ∧
(LENGTH pfx + 1 + LENGTH sfx ≤ LENGTH y' ⇒ ¬isSuffix (NTS B::sfx) y') ⇒
(derives g)^* x y'``,

SRW_TAC [][rderives_def, left2Right] THEN
`rule lhs rhs ∈ l2rRules A B (set (rules g))` by METIS_TAC [mem_in] THEN
`rule lhs' rhs' ∈ l2rRules A B (set (rules g))` by METIS_TAC [mem_in] THEN
FULL_SIMP_TAC (srw_ss()) [derives_def] THEN
FULL_SIMP_TAC (srw_ss()) [l2rRules] THEN SRW_TAC [][] THEN1

METIS_TAC [slemma1_4, startSym_def, rules_def, MEM, MEM_APPEND, rgr_r9eq] THEN1
...
SRW_TAC [][] THEN
 METIS_TAC [slemma1_4, startSym_def, rules_def, MEM, MEM_APPEND, rgr_r9eq,
	    listNtEq']) THEN1
...
(FULL_SIMP_TAC (srw_ss()) [newAprods] THEN
SRW_TAC [][] THEN
`(s1++y = pfx) ∧ (sfx=s2)` by METIS_TAC [listNtEq', APPEND_ASSOC] THEN
SRW_TAC [][] THEN
 `(s1'= s1++y) ∧ (lhs'=B) ∧ (s2=s2')` by METIS_TAC [listNtEq', APPEND_ASSOC] THEN
SRW_TAC [][] THEN
FULL_SIMP_TAC (srw_ss()) [bprods] THEN
SRW_TAC [][] THEN1
(`(derives g) (s1++[NTS A]++s2) (s1 ++ [NTS A] ++ rhs' ++s2)`
 by METIS_TAC [res1, derives_same_append_left, derives_same_append_right,
	       APPEND, APPEND_ASSOC] THEN
 `(derives g) (s1 ++ [NTS A] ++ rhs' ++ s2) (s1 ++ y ++ rhs' ++ s2)`
 by METIS_TAC [res1, derives_same_append_left, derives_same_append_right,
	       APPEND, APPEND_ASSOC] THEN
 METIS_TAC [RTC_RULES]) THEN

 FULL_SIMP_TAC (srw_ss()++ARITH_ss) [] THEN
 FULL_SIMP_TAC (srw_ss()) [isSuffix_def, REVERSE_APPEND, IS_PREFIX_APPEND] THEN
 METIS_TAC [APPEND, APPEND_ASSOC]) THEN

FULL_SIMP_TAC (srw_ss()) [bprods] THEN SRW_TAC [][]);


val ntdlNewgImpg = store_thm
("ntdlNewgImpg",

``left2Right A B g g' ∧ rderives g' ⊢ dl ◁ (pfx ++ [NTS B] ++ sfx) → y ∧
isWord sfx ∧
(∀e. e ∈ dl ⇒ ∃pfx. e = pfx ++ [NTS B] ++ sfx) ∧
(∀e1 e2 p s.
 (dl = p ++ [e1; e2] ++ s) ⇒ LENGTH e2 ≥ LENGTH e1) ∧
rderives g' x (pfx ++ [NTS B] ++ sfx) ∧
¬MEM (NTS B) x ∧
rderives g' y y' ∧
(LENGTH y ≤ LENGTH y' ⇒ ¬isSuffix ([NTS B] ++ sfx) y') ⇒
(derives g)^*  x  y'``,

SRW_TAC [][] THEN
IMP_RES_TAC ldBstream THEN
FIRST_X_ASSUM (Q.SPECL_THEN [`y`,`pfx`,`[]:(α,β) symbol list`,`g'`] MP_TAC) THEN
SRW_TAC [][] THEN
Cases_on `LENGTH dl' > 1`
THENL[
      `ntderivr (NTS B) dl'` by METIS_TAC [ntderivr] THEN
      `∃dl''. derives g ⊢ dl'' ◁ [NTS A] → ([NTS A] ++ FRONT y'')`
      by METIS_TAC [ntdlg'Impg] THEN
      `LAST dl' = y''` by FULL_SIMP_TAC (srw_ss()) [listderiv_def] THEN
      `LAST (MAP (λe. addLast e sfx) (MAP (addFront pfx) dl')) = y` by
      FULL_SIMP_TAC (srw_ss()) [listderiv_def] THEN
      `dl' ≠ []` by (Cases_on `dl'` THEN FULL_SIMP_TAC (srw_ss()) [listderiv_def]) THEN
      ...
      FULL_SIMP_TAC (srw_ss()) [addFront_def,addLast_def] THEN
      SRW_TAC [][] THEN
      `LAST dl' ≠ []` by METIS_TAC [elemNotNilRd,MEM] THEN
      Cases_on `LAST dl'` THEN FULL_SIMP_TAC (srw_ss()) [] THEN SRW_TAC [][] THEN
      FULL_SIMP_TAC (srw_ss()) [rderives_def] THEN
      SRW_TAC [][] THEN
      `rule lhs rhs ∈ l2rRules A B (set (rules g))` by METIS_TAC [mem_in,
								  left2Right] THEN
      FULL_SIMP_TAC (srw_ss()) [l2rRules] THEN1
      METIS_TAC [slemma1_4, rules_def, startSym_def, rgr_r9eq,left2Right,
		 MEM, MEM_APPEND] THEN1

      (FULL_SIMP_TAC (srw_ss()) [newAprods] THEN
      SRW_TAC [][] THEN
      `(s1 ++ y = pfx) ∧ (sfx = s2)` by METIS_TAC [listNtEq', APPEND_ASSOC] THEN
      SRW_TAC [][] THEN
      `MEM (h::t) dl'` by METIS_TAC [memld] THEN
      FULL_SIMP_TAC (srw_ss()) [ntderivr] THEN
      `LAST (h::t) = NTS B` by METIS_TAC [] THEN
      `rule lhs' rhs' ∈ l2rRules A B (set (rules g))` by METIS_TAC [mem_in,
								    left2Right] THEN
      FULL_SIMP_TAC (srw_ss()) [l2rRules] THEN
      Q.ABBREV_TAC `l = h::t` THEN
      `l = FRONT l ++ [NTS B]` by METIS_TAC [APPEND_FRONT_LAST, NOT_CONS_NIL] THEN
       SRW_TAC [][] THEN1

       (`(s2=s2') ∧ (B = lhs') ∧ (s1' = s1 ++ y ++ FRONT (LAST dl'))`
       by METIS_TAC [APPEND_ASSOC, listNtEq'] THEN
       METIS_TAC [slemma1_4, rules_def, startSym_def, rgr_r9eq,left2Right]) THEN1
	...
	`(s2=s2') ∧ (B = lhs') ∧ (s1' = s1 ++ y ++ FRONT (LAST dl'))`
       ...
       SRW_TAC [][] THEN
       FULL_SIMP_TAC (srw_ss()) [isSuffix_def, REVERSE_APPEND, IS_PREFIX_APPEND] THEN
	`LENGTH (h::t) = LENGTH (FRONT (LAST dl') ++ [NTS B])` by METIS_TAC [] THEN
	FULL_SIMP_TAC (srw_ss()++ARITH_ss) [] THEN
       ...
       FULL_SIMP_TAC (srw_ss()) [bprods] THEN
       SRW_TAC [][] THEN1
       (`(s2=s2')  ∧ (s1' = s1 ++ y ++ FRONT (LAST dl'))`
       by METIS_TAC [APPEND_ASSOC, listNtEq'] THEN
       SRW_TAC [][] THEN
       `(derives g)^* [NTS A] (NTS A::FRONT (LAST dl'))`
       by METIS_TAC [memld, rtc2list_mem, RTC_RULES] THEN
       `(derives g)^* ([NTS A]++rhs') (NTS A::FRONT (LAST dl')++rhs')`
       by METIS_TAC [rtc_derives_same_append_right, APPEND, APPEND_ASSOC] THEN
       `(derives g)^* (s1++[NTS A]++rhs'++s2)
             (s1++NTS A::FRONT (LAST dl')++rhs'++s2)`
       by METIS_TAC [rtc_derives_same_append_right, APPEND, APPEND_ASSOC,
		     rtc_derives_same_append_left] THEN
       `(derives g)^* (NTS A::FRONT (LAST dl')++rhs') (y++FRONT (LAST dl')++rhs')`
       by METIS_TAC [derives_same_append_right, APPEND, APPEND_ASSOC,
		     res1, RTC_RULES] THEN
       `(derives g)^* (s1++NTS A::FRONT (LAST dl')++rhs'++s2)
	     (s1++y++FRONT (LAST dl')++rhs'++s2)`
       by METIS_TAC [rtc_derives_same_append_right, APPEND, APPEND_ASSOC,
		     res1, RTC_RULES,rtc_derives_same_append_left] THEN
       `(derives g) (s1 ++ [NTS A] ++ s2) (s1 ++ [NTS A] ++ rhs' ++ s2)`
       by METIS_TAC [res1, derives_same_append_right, derives_same_append_left,
		     APPEND, APPEND_ASSOC] THEN
       `(derives g)^* (s1 ++ [NTS A] ++ s2)
         (s1++ y ++ FRONT (LAST dl') ++ rhs'++s2)` by
       METIS_TAC [RTC_RULES,APPEND, APPEND_ASSOC,
		  RTC_RTC] THEN
       METIS_TAC [rtc2list_exists', APPEND_ASSOC, APPEND]) THEN
...
Cases_on `LENGTH dl > 1`
THENL[
      `∃(dl1:(α,β) symbol list list) (dl2:(α,β) symbol list list)
      (dl3:(α,β) symbol list list).
           (dl = dl1 ++ dl2 ++ dl3) ∧
           (ldNumNt (NTS A) dl1 = 0) ∧
           (∀e1 e2 p s.
              (dl2 = p ++ [e1; e2] ++ s) ⇒ LENGTH e2 ≥ LENGTH e1) ∧
           ∃pfx.
	   isWord pfx ∧
             ...
             (dl3 ≠ ([]:(α,β) symbol list list) ⇒
              ...
              ¬((pfx ++ [NTS A]) ≼ (HD dl3)))` by
      (IMP_RES_TAC dlsplit THEN METIS_TAC []) THEN
      SRW_TAC [][] THEN
...
(* This is horribly structured. KLS *)

val dlsplitB = store_thm
("dlsplitB",
``∀dl x y.
    rderives g ⊢ dl ◁ x → y ∧ rdNumNt (NTS B) dl ≠ 0 ∧ LENGTH dl > 1
   ⇒
    ∃dl1 dl2 dl3. 
       (dl = dl1++dl2++dl3) ∧ (rdNumNt (NTS B) dl1 = 0) ∧
       (∀e1 e2 p s.(dl2 = p ++ [e1; e2] ++ s) ⇒ LENGTH e2 ≥ LENGTH e1) ∧
       ∃sfx. EVERY isTmnlSym sfx  ∧ 
...
Cases_on `dl` THEN1
FULL_SIMP_TAC (srw_ss()) [listderiv_def] THEN
Cases_on `t` THEN FULL_SIMP_TAC (srw_ss()) [] THEN SRW_TAC [][]
>- (*1*)
   (FULL_SIMP_TAC (srw_ss()) [listderiv_def] THEN
     SRW_TAC [][] THEN
     FULL_SIMP_TAC (srw_ss()) [rderives_def] THEN SRW_TAC [][] THEN
     Cases_on `∃rst.rhs=rst++[NTS B]`
      >- (`rdNumNt (NTS B) [s1++rhs++s2] ≠ 0` 
           by (SRW_TAC [][rdNumNt] THEN
               FULL_SIMP_TAC (srw_ss()) [rightntm] THEN
               METIS_TAC [NOT_EVERY, APPEND, APPEND_ASSOC]) THEN
          Cases_on `rdNumNt (NTS B) [s1++[NTS lhs]++s2] = 0`
           >- (MAP_EVERY Q.EXISTS_TAC [`[s1++[NTS lhs]++s2]`,`[s1++rhs++s2]`,`[]`] THEN
               SRW_TAC [][] THEN1
	         (Cases_on `p` THEN FULL_SIMP_TAC (srw_ss()) []) THEN METIS_TAC [rnnSing])
           >- (`lhs=B` by METIS_TAC [rdNumNtEq] THEN SRW_TAC [][] THEN
               MAP_EVERY Q.EXISTS_TAC 
                   [`[]`,`[s1++[NTS B]++s2;s1++rst++[NTS B]++s2]`, `[]`] THEN
               SRW_TAC [][rdNumNt] THEN
               IMP_RES_TAC rnnSing THEN SRW_TAC [][] THEN
               `pfx=s1` by METIS_TAC [listNtEq'] THEN SRW_TAC [][] THEN
               `s2=sfx` by METIS_TAC [APPEND_11, APPEND_ASSOC] THEN SRW_TAC [][] THEN
               `s2=sfx'` by METIS_TAC [listNtEq',APPEND,APPEND_ASSOC] THEN SRW_TAC [][] THEN
               `pfx++rst = pfx'` by METIS_TAC [APPEND_11, APPEND_ASSOC,APPEND] THEN
                SRW_TAC [][] 
                 THEN1 (Cases_on `p` THEN FULL_SIMP_TAC (srw_ss()) [] THEN
                        SRW_TAC [][] THEN1
	                    (Cases_on `rst` THEN SRW_TAC [ARITH_ss][]) THEN
                           Cases_on `t` THEN FULL_SIMP_TAC (srw_ss()) []) THEN
	       METIS_TAC [APPEND_ASSOC]))
      ...
      >- (FULL_SIMP_TAC (srw_ss()) [] THEN
	      ...
	      FULL_SIMP_TAC (srw_ss()) [REVERSE_APPEND]
              >> qpat_x_assum `!rst. ~(rhs = rst ++ [NTS B])` mp_tac
              >> WEAKEN_TAC is_forall
              >> qpat_x_assum `COND _ _ _ = []` kall_tac
              >> qpat_x_assum `~(COND _ _ _ = _)` kall_tac
              >> qpat_x_assum `L1 ++ L2 ++ L3 = _ ++ _ ++ _` kall_tac
              >> qpat_x_assum `_ IN _` kall_tac
              >> qpat_x_assum `isWord _` kall_tac
              >> disch_tac
              >> `~(REVERSE rhs = REVERSE (REVERSE l ++ [NTS B]))` by metis_tac [REVERSE_11]
              >> full_simp_tac std_ss [REVERSE_APPEND]
              >> full_simp_tac std_ss [REVERSE_DEF,APPEND,REVERSE_REVERSE]
              >> Cases_on `REVERSE rhs`
                 >- (full_simp_tac list_ss [] >> rw_tac list_ss [])
                 >- (full_simp_tac std_ss [APPEND,CONS_11] 
                      >- metis_tac []
                      >- (rw_tac list_ss []
                          >> `~(REVERSE rhs = REVERSE (REVERSE t ++ [NTS B]))` 
                                by metis_tac [REVERSE_11]
                          >> full_simp_tac std_ss [REVERSE_APPEND]
                          >> full_simp_tac std_ss [REVERSE_DEF,APPEND,REVERSE_REVERSE]))
	     )
	      ...
	      SRW_TAC [][] THEN
	      FULL_SIMP_TAC (srw_ss()) [] THEN SRW_TAC [][] THEN
	      Q.PAT_X_ASSUM `REVERSE s2 ++ NTS B::REVERSE pfx' =
...
>- (*2*)
   (IMP_RES_TAC listDerivHdBrk THEN
    Cases_on `rdNumNt (NTS B) (h'::h''::t') = 0` 
     >- (*2.1*)
        (FULL_SIMP_TAC (srw_ss()) [] THEN
     ...
     >- (*2.2*)
        (FULL_SIMP_TAC (arith_ss) [] THEN
         ...
         Cases_on `rdNumNt (NTS B) [h] = 0` THEN1
           (MAP_EVERY Q.EXISTS_TAC [`h::dl1`,`dl2`,`dl3`] THEN SRW_TAC [][] THEN
            METIS_TAC [APPEND, rdNumNtApp]) THEN
         Cases_on `dl1=[]` THEN SRW_TAC [][]
          >- (*2.2.1*)
             (Cases_on `dl3=[]` THEN FULL_SIMP_TAC (srw_ss()) [] THEN 
              SRW_TAC [][]
               >- (*2.2.1.1*)
                  (FULL_SIMP_TAC (srw_ss()) [rderives_def] THEN
                   SRW_TAC [][] THEN
	           Cases_on `rhs=[]` THEN FULL_SIMP_TAC (srw_ss()) []
		        ...
		        SRW_TAC [][] THEN1
                           (Cases_on `p` THEN FULL_SIMP_TAC (srw_ss()) []) THEN
		        Q.EXISTS_TAC `s2` THEN SRW_TAC [][] 
                           THEN1 METIS_TAC [rdNumNtEq] THEN
		        FULL_SIMP_TAC (arith_ss) [])
		    >- (*2.2.1.1.2*)
                       (Cases_on `sfx=s2` THEN SRW_TAC [][]
                         >- (MAP_EVERY Q.EXISTS_TAC
                              [`[]`,`[s1++[NTS lhs]++s2]++(s1 ++ rhs ++s2)::h''::t'`,`[]`] THEN
			     SRW_TAC [][] THEN1
			       (Cases_on `p` THEN FULL_SIMP_TAC (srw_ss()) [] THEN
                                SRW_TAC [][] THEN1
			          (Cases_on `rhs` THEN
			     ...
			     Q.EXISTS_TAC `s2` THEN SRW_TAC [][] 
			       >- METIS_TAC [rdNumNtEq] 
			       >- (`∃pfx. s1++rhs++s2 = pfx++[NTS B]++sfx` 
                                      by METIS_TAC [] 
                                    >> strip_tac
				    >> full_simp_tac list_ss 
                                         [isSuffix_def,REVERSE_APPEND,IS_PREFIX_APPEND]
                                    >> `REVERSE rhs ++ REVERSE s1 = [NTS B] ⧺ l`
                                       by metis_tac[APPEND_ASSOC,APPEND_11]
                                    >> `REVERSE (REVERSE rhs ++ REVERSE s1) = 
                                        REVERSE ([NTS B]++l)` 
                                        by metis_tac[]
		       ...
		       FULL_SIMP_TAC (srw_ss()) [] 
		        ...
		        >- (*2.2.1.2.1.2*)
			        ...
			        Q.EXISTS_TAC `s2` THEN SRW_TAC [][] 
			          ...
			          >- METIS_TAC [APPEND_ASSOC] 
			          >- (FULL_SIMP_TAC (srw_ss()) 
                                        [isSuffix_def, IS_PREFIX_APPEND] THEN
   ...
   (Cases_on `dl1` THEN FULL_SIMP_TAC (srw_ss()) [] THEN SRW_TAC [][] THEN
     ...
     SRW_TAC [][rdNumNt] 
      ...
      >- (FULL_SIMP_TAC (srw_ss()) [rderives_def] THEN SRW_TAC [][] THEN
          Q.EXISTS_TAC `s2` THEN SRW_TAC [][] THEN1
              METIS_TAC [rdNumNtEq] THEN
          `¬∃pfx sfx. EVERY isTmnlSym sfx ∧ 
                       (s1++rhs++s2 = pfx ++ [NTS B]++sfx)`
             by METIS_TAC [rnnSing,rdNumNtApp,APPEND] THEN
          FULL_SIMP_TAC (srw_ss()) [IS_PREFIX_APPEND, isSuffix_def] THEN
          SPOSE_NOT_THEN ASSUME_TAC THEN SRW_TAC [][] THEN
          `REVERSE s2 ⧺ [NTS B] ⧺ l = REVERSE (REVERSE l ++ [NTS B] ++ s2)` 
            by (rw_tac list_ss [REVERSE_APPEND, REVERSE] 
                >> metis_tac [APPEND,APPEND_ASSOC])
          >> metis_tac [REVERSE_11,NOT_EVERY]))
	)
   )
);


val dlNotMemB = store_thm
("dlNotMemB",
``∀dl x y.
left2Right A B g g' ∧ (rderives g') ⊢ dl ◁ x → y ∧ ¬MEM (NTS B) x ∧
(rdNumNt (NTS B) dl = 0) ⇒
¬MEM (NTS B) y``,
...
`h = x` by FULL_SIMP_TAC (srw_ss()) [listderiv_def] THEN
SRW_TAC [][] THEN
`rdNumNt (NTS B) (h'::t) =0` by METIS_TAC [rdNumNtApp, APPEND, APPEND_ASSOC] THEN
FIRST_X_ASSUM (Q.SPECL_THEN [`h'`,`y`] MP_TAC) THEN SRW_TAC [][] THEN
FULL_SIMP_TAC (srw_ss()) [rderives_def] THEN
SRW_TAC [][] THEN
SPOSE_NOT_THEN ASSUME_TAC THEN
SRW_TAC [][] THEN
...
METIS_TAC [slemma1_4, rgr_r9eq, rules_def, startSym_def] THEN1

(FULL_SIMP_TAC (srw_ss()) [newAprods] THEN
 SRW_TAC [][] THEN
 `rdNumNt (NTS B) [s1++y'++[NTS B]++s2] = 0` by METIS_TAC [rdNumNtApp, APPEND,
							   APPEND_ASSOC] THEN
 METIS_TAC [rnnSing, APPEND, APPEND_ASSOC]) THEN

FULL_SIMP_TAC (srw_ss()) [bprods] THEN
METIS_TAC [slemma1_4, rgr_r9eq, rules_def, startSym_def] THEN

FULL_SIMP_TAC (srw_ss()) []);



val dlNotMemBrdNum = store_thm
("dlNotMemBrdNum",
``∀dl x.
      left2Right A B g g' ∧ (rderives g') ⊢ dl ◁ x → y ∧
      (LENGTH (FILTER (\e.e=NTS B) x) = 1) ∧
      rightntm (NTS B) x ∧
      (∀e. e ∈ dl ⇒ ∃pfx. e = pfx ++ [NTS B] ++ sfx) ∧
      rderives g' y z ∧ isWord sfx ∧
     (LENGTH y ≤ LENGTH z ⇒ ¬isSuffix ([NTS B] ++ sfx) z) 
  ⇒ ¬MEM (NTS B) z``,
...
Cases_on `dl` THEN FULL_SIMP_TAC (srw_ss()) [] 
 >- (FULL_SIMP_TAC (srw_ss()) [listderiv_def] THEN
     ...
     FULL_SIMP_TAC (srw_ss()) [rderives_def] THEN
     SRW_TAC [][] THEN
     `(s1 = pfx) ∧(lhs = B) ∧ (s2=sfx)` by METIS_TAC [listNtEq'] THEN
     SRW_TAC [][] 
      >- (SPOSE_NOT_THEN ASSUME_TAC THEN
          ...
          DECIDE_TAC)
      >- (FULL_SIMP_TAC (srw_ss()) [left2Right] THEN
          `rule B rhs ∈ l2rRules A B (set (rules g))` 
             by METIS_TAC [mem_in] THEN
          FULL_SIMP_TAC (srw_ss()) [l2rRules] 
          >- METIS_TAC [slemma1_4, rgr_r9eq, rules_def, startSym_def, left2Right]
          >- (FULL_SIMP_TAC (srw_ss()) [newAprods] THEN
              METIS_TAC [slemma1_4, rgr_r9eq, rules_def, startSym_def, left2Right])
          >- (FULL_SIMP_TAC (srw_ss()) [bprods] THEN1
                 METIS_TAC [slemma1_4, rgr_r9eq, rules_def, 
                    startSym_def, APPEND, APPEND_ASSOC] THEN
              SRW_TAC [][] THEN
              FULL_SIMP_TAC (srw_ss()++ARITH_ss) [] THEN
              FULL_SIMP_TAC (srw_ss()) [isSuffix_def, REVERSE_APPEND, IS_PREFIX_APPEND] THEN
              METIS_TAC [APPEND, APPEND_ASSOC]))
 ...
 >- (IMP_RES_TAC listDerivHdBrk THEN
     ...
     FULL_SIMP_TAC (srw_ss()) [rightntm] THEN
     SRW_TAC [][] THEN
     `sfx = sfx'` by METIS_TAC [listNtEq'] THEN
     SRW_TAC [][] THEN
    `LENGTH (FILTER (λe. e = NTS B) h') = 1` 
 by (`∃pfx. h' = pfx ++ [NTS B] ++ sfx` by METIS_TAC [MEM] THEN
     SRW_TAC [][] THEN
     FULL_SIMP_TAC (srw_ss()) [rderives_def] THEN
     SRW_TAC [][] THEN
     `(s1' = pfx) ∧ (lhs'=B) ∧ (s2'=sfx)` by METIS_TAC [listNtEq'] THEN
     SRW_TAC [][] THEN
     FULL_SIMP_TAC (srw_ss()) [FILTER_APPEND] THEN
     `(LENGTH (FILTER (λe. e = NTS B) pfx) = 0) ∧
      (LENGTH (FILTER (λe. e = NTS B) s2') = 0)`  by DECIDE_TAC THEN
     `¬(MEM (NTS B) s2)` 
       by (SPOSE_NOT_THEN ASSUME_TAC THEN 
           ...
           FULL_SIMP_TAC (srw_ss()) [isTmnlSym_def]) THEN
     FULL_SIMP_TAC (srw_ss()) [left2Right] THEN
     `rule B rhs' ∈ l2rRules A B (set (rules g))` by METIS_TAC [mem_in] THEN
     FULL_SIMP_TAC (srw_ss()) [l2rRules] 
     >- METIS_TAC [slemma1_4, rgr_r9eq, rules_def, startSym_def, left2Right]
     >- (FULL_SIMP_TAC (srw_ss()) [newAprods] THEN
         SRW_TAC [][] THEN
     ...
     >- (FULL_SIMP_TAC (srw_ss()) [bprods] THEN1
           (Cases_on `rhs'=[]`THEN FULL_SIMP_TAC (srw_ss()) [] THEN1
              (SRW_TAC [][] THEN
               FULL_SIMP_TAC (srw_ss()) [FILTER_APPEND]) THEN
            `rhs' = FRONT rhs' ++ [LAST rhs']` 
                by METIS_TAC [APPEND_FRONT_LAST] THEN
            SRW_TAC [][] THEN
            `pfx' = pfx ++ FRONT rhs'` 
               by METIS_TAC [lastElemEq, APPEND_ASSOC, APPEND_11] THEN
            SRW_TAC [][] THEN
            `¬MEM (NTS B) rhs'`
              by METIS_TAC [slemma1_4, rgr_r9eq, rules_def, startSym_def,
                   left2Right, APPEND, APPEND_ASSOC] THEN
            FULL_SIMP_TAC (srw_ss()) [FILTER_APPEND, filter_l2,FILTER_EQ_NIL,EVERY_MEM] THEN
            METIS_TAC [MEM, MEM_APPEND]) THEN
         SRW_TAC [][] THEN
         `pfx ++ a  = pfx'` by METIS_TAC [lastElemEq, APPEND_ASSOC, APPEND_11] THEN
         SRW_TAC [][] THEN
         `¬MEM (NTS B) a` 
          by METIS_TAC [slemma1_4, rgr_r9eq, rules_def, startSym_def, left2Right,
	                APPEND, APPEND_ASSOC] THEN
         FULL_SIMP_TAC (srw_ss()) [FILTER_APPEND, filter_l2,FILTER_EQ_NIL,EVERY_MEM]))
 >> metis_tac [])
);

val rdFilterLen = store_thm
("rdFilterLen",
``left2Right A B g g' ∧ 
  rderives g' x y ∧ 
  ¬MEM (NTS B) x ∧
  MEM (NTS B) y ⇒ (LENGTH (FILTER (\e. e=NTS B) y) = 1)``,

SRW_TAC [][rderives_def, left2Right] THEN
FULL_SIMP_TAC (srw_ss()) [FILTER_APPEND]
 >- metis_tac [] 
 >- (`LENGTH (FILTER (λe. e = NTS B) s1) = 0` by SRW_TAC [][filter_l2] THEN
     `LENGTH (FILTER (λe. e = NTS B) s2) = 0` by SRW_TAC [][filter_l2] THEN
     SRW_TAC [][] THEN
     `rule lhs rhs ∈ l2rRules A B (set (rules g))` by METIS_TAC [mem_in] THEN
     FULL_SIMP_TAC (srw_ss()) [l2rRules] 
     >- METIS_TAC [slemma1_4, rgr_r9eq, rules_def, startSym_def]
     >- (FULL_SIMP_TAC (srw_ss()) [newAprods] THEN
         SRW_TAC [][] THEN
        `¬MEM (NTS B) y` by METIS_TAC [slemma1_4, rgr_r9eq, rules_def, startSym_def] THEN
        FULL_SIMP_TAC (srw_ss()) [FILTER_APPEND, filter_l2]) 
     >- (FULL_SIMP_TAC (srw_ss()) [bprods] THEN
         METIS_TAC [slemma1_4, rgr_r9eq, rules_def, startSym_def]))
 >- metis_tac []
);
...
("ldNewgImpg",
``∀x y.left2Right A B g g' ∧ rderives g' ⊢ dl ◁ x → y ∧
 ¬MEM (NTS B) x ∧ ¬MEM (NTS B) y
 ⇒
 ∃dl'.derives g ⊢ dl' ◁ x → y``,
...
THENL[
      `∃dl1 dl2 dl3.
           ...
           ∃sfx.
             ...
             (dl3 ≠ [] ⇒
              LENGTH (LAST dl2) ≤ LENGTH (HD dl3) ⇒
              ¬(isSuffix ([NTS B]++sfx) (HD dl3)))` by
      (IMP_RES_TAC dlsplitB THEN METIS_TAC []) THEN
      ...
      `dl1 ≠ []` by
      (Cases_on `dl1` THEN Cases_on `dl2` THEN FULL_SIMP_TAC (srw_ss()) [] THEN
       SRW_TAC [][] THEN
       METIS_TAC [listderiv_def, MEM, MEM_APPEND, HD]) THEN
      `rderives g' ⊢ dl1 ◁ HD dl1 → LAST dl1`
      by METIS_TAC [ldsubderivs,APPEND_ASSOC] THEN
      `HD dl1 = x` by (Cases_on `dl1` THEN
		       FULL_SIMP_TAC (srw_ss()) [listderiv_def]) THEN
      SRW_TAC [][] THEN
      `¬MEM (NTS B) (LAST dl1)` by METIS_TAC [dlNotMemB] THEN
      `dl2 = HD dl2::TL dl2` by METIS_TAC [CONS,NULL_EQ_NIL] THEN
      `rderives g' (LAST dl1) (HD dl2)` by METIS_TAC [ldMemRel',APPEND,
						     APPEND_FRONT_LAST,
						     APPEND_ASSOC] THEN
      `(LENGTH (FILTER (\e.e=NTS B) (HD dl2)) = 1)`
      by METIS_TAC [rdFilterLen,MEM, MEM_APPEND] THEN

      `rderives g' ⊢ dl2 ◁ HD dl2 → LAST dl2`
      by METIS_TAC [ldsubderivs,APPEND_ASSOC] THEN
      `dl3 ≠ []`
      by (SRW_TAC [][] THEN
	  SPOSE_NOT_THEN ASSUME_TAC THEN SRW_TAC [][] THEN
      ...
      `rderives g' (LAST dl2) (HD dl3)` by METIS_TAC [ldMemRel',APPEND_FRONT_LAST,
						      APPEND_ASSOC,APPEND] THEN
      `MEM (HD dl2) dl2` by METIS_TAC [memld] THEN
      `rightntm (NTS B) (HD dl2)` by METIS_TAC [rightntm] THEN
      `¬(NTS B ∈ (HD dl3))` by METIS_TAC [dlNotMemBrdNum, NOT_CONS_NIL] THEN
      `derives g ⊢ dl1 ◁ HD dl1 → LAST dl1` by METIS_TAC [blkNilNewgImpg] THEN
      `∃dl'. derives g ⊢ dl' ◁ HD dl3 → y` by METIS_TAC [] THEN
      `(derives g)^* (LAST dl1) (HD dl3)` by METIS_TAC [ntdlNewgImpg] THEN
      IMP_RES_TAC rtc2list_exists' THEN
      METIS_TAC [listderivTrans],
...
THENL[
      IMP_RES_TAC derivesImpLderives THEN
      IMP_RES_TAC rtc2list_exists' THEN
      `∃dl. derives g' ⊢ dl ◁ [NTS (startSym g)] → x`
      by METIS_TAC [ldgrImpNewg] THEN
      Cases_on `dl` THEN FULL_SIMP_TAC (srw_ss()) [listderiv_def, left2Right] THEN
      SRW_TAC [][] THEN
      METIS_TAC [rtc2listRtcHdLast, NOT_CONS_NIL, HD],

      IMP_RES_TAC derivesImpRderives THEN
      `¬(((NTS B):(α,β) symbol) ∈ [NTS (startSym g')])` by
      (FULL_SIMP_TAC (srw_ss()) [left2Right] THEN
       METIS_TAC [slemma1_4]) THEN
      `¬(((NTS B):(α,β) symbol) ∈ x)` by
	 (SPOSE_NOT_THEN ASSUME_TAC THEN
	  FULL_SIMP_TAC (srw_ss()) [rgr_r9eq] THEN
	  SRW_TAC [][] THEN
	  FULL_SIMP_TAC (srw_ss()) [isTmnlSym_def]) THEN
