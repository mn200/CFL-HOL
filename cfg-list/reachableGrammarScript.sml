...
open pred_setTheory grammarDefTheory listLemmasTheory;
...
val rgr = Define 
`rgr g g' = (∀e.MEM e (rules g') = e ∈ { rule l r | MEM (rule l r) (rules g) ∧ 
...
SRW_TAC [] [] THEN
Cases_on `g` THEN 
FULL_SIMP_TAC (srw_ss()) [startSym_def,rules_def,rgr] THEN
SRW_TAC [][] THEN
...
MAP_EVERY Q.EXISTS_TAC [`s1`,`s2`,`rhs`,`lhs`] THEN
SRW_TAC [] [] THEN
METIS_TAC [rgr_r2,rgr,rules_def,slres,startSym_def]
);

val rgr_r6 = prove(
``rgr g g' ⇒ (derives g [NTS (startSym g)] v = derives g' [NTS (startSym g')] v)``,

METIS_TAC [rgr_subr2,rgr_subr3,eq_srgr,rgr]
);
...
SRW_TAC [] [rules_def,rgr] THEN
RES_TAC THEN FULL_SIMP_TAC (srw_ss()) [] THEN SRW_TAC [][] THEN
`rgr g g'`by FULL_SIMP_TAC (srw_ss()) [rules_def, rgr] THEN
...
SRW_TAC [] [] THEN
FULL_SIMP_TAC (srw_ss()) [derives_def,rgr,rules_def] THEN
RES_TAC THEN FULL_SIMP_TAC (srw_ss()) [] THEN SRW_TAC [][] THEN
