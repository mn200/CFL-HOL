...
(``negr g0 g ⇒ upgr g g' ⇒
 derives g' v v' ⇒ 
 derives g v v'``,
 FULL_SIMP_TAC (srw_ss()) [unitProds,rules_def,derives_def,eq_sneup,
			   upgr,upgr_rules] THEN 
 SRW_TAC [][] THEN
 METIS_TAC []
 );
...
Cases_on `g` THEN
`RTC (derives g) [NTS lhs] [NTS b]` by METIS_TAC [upgr_r2] THEN
...
val upgr_r6 = prove(
``RTC (derives (negr g)) [NTS x] [NTS y] ⇒ negr g0 g ⇒ upgr g g' ⇒
(NTS x IN nonTerminals (negr g)) ⇒ (NTS y IN nonTerminals (negr g))  ⇒  
(NTS x,NTS y) IN (allDeps (negr g))``,
