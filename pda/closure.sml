...
Cases_on `t` THEN
SRW_TAC [][isLeaf_def, root, fringe_def]);
...
Cases_on `h` THEN FULL_SIMP_TAC (srw_ss()) [root, fringe_def] THEN1
METIS_TAC [rtc_derives_same_append_left, APPEND] THEN
`(derives g)^* [NTS n] (MAP TS (fringe (Node n l)))` by METIS_TAC [root] THEN
...
Induct_on `ptl` THEN SRW_TAC [][root, getSymbols_def] THEN
Cases_on `h` THEN FULL_SIMP_TAC (srw_ss()) [root, getSymbols_def]);
...
FULL_SIMP_TAC (srw_ss()) [root, validptree] THEN
 ...
 by (SRW_TAC [][] THEN
     Cases_on `e` THEN 
     FULL_SIMP_TAC (srw_ss()++boolSimps.ETA_ss) [root, fringe_def] THEN
...
Induct_on `x` THEN SRW_TAC [][isTmnlSym_def, getSymbols_def, ts2str_def]);
...
("lpexImpvpt",
``∀l.(∀a b.MEM (a,b) l ⇒  
   (isNonTmnlSym a ⇒ ∃t.validptree g t ∧ (MAP TS (fringe t) = b) ∧ (root t = a)) ∧
      (isTmnlSym a ⇒ ([a]=b))) ⇒
...
SRW_TAC [boolSimps.ETA_ss][validptree, fringe_def, root] THEN
...
DISJ2_TAC THEN
Q.EXISTS_TAC `[NTS (startSym g1)]` THEN
...
SRW_TAC [][Once RTC_CASES1]THEN
DISJ2_TAC THEN
Q.EXISTS_TAC `[NTS (startSym g2)]` THEN
SRW_TAC [][derives_def, rules_def, grUnion] THEN
METIS_TAC [APPEND_NIL]);
...
Theorem 6.2 The CFLs are closed under substitution.

Corollary The CFLs are closed under homomorphisms.
*)
