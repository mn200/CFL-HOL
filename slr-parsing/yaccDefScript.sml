...
THENL[
      ...
      THENL[
	    ...
	    METIS_TAC [delete_not_mem,MEM,rmd_mem_list,not_mem_delete,delete_mem_list]
	    ]
      ])
...
THENL[
      ...
      `LENGTH (t ++ [NTS st] ++ h'::t') = LENGTH [TS eof]` by METIS_TAC [] THEN
      FULL_SIMP_TAC (srw_ss()) [] THEN
      FULL_SIMP_TAC (arith_ss) []	    
      ])
...
THENL[
      ...
      FULL_SIMP_TAC (srw_ss()) []
      THENL[
	    `NTS lhs = TS eof` by METIS_TAC [last_elem] THEN
	    FULL_SIMP_TAC (srw_ss()) [],
...
red = reduce g
in
if (?pfx itl sym.(trans g (initItems g (rules g),pfx) = SOME itl) /\
let
    (s = sg itl sym) and
...
`(s = itemLhs (item s' ([], l))) /\ (q ++ r = itemFstRhs(item s' ([], l)) ++ itemSndRhs  (item s' ([], l)))` by METIS_TAC [itemEqRuleExists] THEN
      ...
      Cases_on `pfx0=pfx` THENL[
        ...
        `isSuffix s1 sfx0`
           by (`isSuffix s1 sfx0 \/ isSuffix sfx0 s1`
               ...
               `s2' ++ h::t' = t ++ [NTS Nt1]`
                  by METIS_TAC [APPEND_11, APPEND_ASSOC] THEN
               `LAST (h::t') = NTS Nt1` by METIS_TAC [NOT_CONS_NIL, last_elem, last_append] THEN
               `MEM (LAST (h::t')) (h::t')`
                 by METIS_TAC [MEM_APPEND, APPEND_FRONT_LAST, 
                               MEM] THEN 
               METIS_TAC [isTmnlSymML, EVERY_MEM, EVERY_APPEND]) THEN
...
SRW_TAC [][] THEN
Cases_on `l2` THEN SRW_TAC [] [iclosure1] 
THENL[
      METIS_TAC [],

      Cases_on `h` THEN SRW_TAC [] [iclosure1] 
      THENL[
	    METIS_TAC [],
...
Cases_on `sym` THEN FULL_SIMP_TAC (srw_ss()) [reduce],
Cases_on `h` THEN Cases_on `p` THEN
Cases_on `r` THEN
FULL_SIMP_TAC (srw_ss() ++ DNF_ss) [doReduce, LET_THM, option_case_rwt, list_case_rwt, pairTheory.FORALL_PROD] THENL[
...
METIS_TAC [reduce_mem,validItl, APPEND, CONS, EVERY_DEF, MEM, validItl_append, rgr_r9eq, APPEND_NIL],
